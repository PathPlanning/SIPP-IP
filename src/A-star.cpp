...
class Bot {
public:
    int x, y, o, v, t, parentID;

    Bot(int x, int y, int o, int v, int t) : x(x), y(y), o(o), v(v), t(t) {
        parentID = -1;
    }

    Bot(int x, int y, int o, int v, int t, int parentID) : x(x), y(y), o(o), v(v), t(t), parentID(parentID) {}
    ...
    friend ostream &operator<<(ostream &output, const Bot &p) {
        output << "x=" << p.x << ", y=" << p.y << ", o=" << p.o << ", v=" << p.v << ", t=" << p.t << ", parentID=" << p.parentID;
        return output;
    }
};
...
bool operator<(const Bot &a, const Bot &b) {
    if (a.t + (abs(a.x - end_x) + abs(a.y - end_y))*minimalTransitionCost != b.t + (abs(b.x - end_x) + abs(b.y - end_y))*minimalTransitionCost) {
        return a.t + (abs(a.x - end_x) + abs(a.y - end_y))*minimalTransitionCost < b.t + (abs(b.x - end_x) + abs(b.y - end_y))*minimalTransitionCost;
    }
...
set<int> CLOSED[MXH][MXW][MXO][MXV];

bool outMap(int x, int y) {
    return x < 0 || x >= H || y < 0 || y >= W;
}

bool checkCLOSED(int x, int y, int o, int v, int t) {
    return CLOSED[x][y][o][v].find(t)!=CLOSED[x][y][o][v].end();
}

vector<Bot> applyPrimitive(int x, int y, int t, Primitive mp) {
    ...
    for (auto mv: mp.mvs) {
        ...
        t = min(INF, t + (mv.ftt - past_time));
        past_time = mv.ftt;
        auto itr = rsrv_tbl[xx][yy].lower_bound({t, INF - 1});
        auto itr_prev = itr;
        --itr_prev;
        if(itr_prev->second >= t || itr->first-mv.swt<= t) {
            return {};
        }
    }
    return {Bot(xx, yy, mp.o, mp.v,min(INF, t+mp.mvs.back().swt))};
}

void generateSuccessors(int x, int y, int o, int v, int t, vector<Bot> &succs) {
    for (auto &mp: motion_primitives[o][v]) {
        auto vBots = applyPrimitive(x, y, t, mp);
...
pair<int, bool> search(int st_x, int st_y, int st_o, int st_v, int st_t = 0) {
    ...
    --it;
    if (st_tupper < st_t || it->second >= st_t) {
        cerr << "Error, the initial state falls in an obstacle!!" << endl;
        exit(0);
    }
    OPEN.clear();
    CLOSED_vec.clear();
    OPEN.insert(Bot(st_x, st_y, st_o, st_v, st_t, -1));
    int x, y, o, v, t, id;
    int cntNodes = 0;
    while (!OPEN.empty() && cntNodes < MAX_NUM_NODES) {
        ...
        --cntNodes;
        x = tmp.x, y = tmp.y, o = tmp.o, v = tmp.v, t = tmp.t;
        if (checkCLOSED(x, y, o, v, t)) {
            continue;
        }
        CLOSED[x][y][o][v].insert(t);
        ++cntNodes;
        id = CLOSED_vec.size();
        ...
        vector<Bot> succs;
        generateSuccessors(x, y, o, v, t, succs);
        for (auto &it: succs) {
            it.parentID = id;
            OPEN.insert(it);
            ++cntNodes;
...
int main() {
    cout<<"Astar started... map:"<<map<<endl;
    ofstream out;
    fillActions();
    ...
    for(int f = 0; f<factors.size(); ++f){
        ...
        out.open("../results/results Astar/res-" + map + "Astar-obs"+to_string(num_of_obstacles)+".txt");
        for (testNum = 0; testNum < NumOfTests; ++testNum) {
            ...
            out << "Success:" << ans.second << ", Cost:"<< CLOSED_vec[ans.first].t << ", Runtime:" << diffTime << "\n";
            if (ans.second) {
